...
export default function Home() {
  ...
  if (loading) return <div style={{ color: "#fff", textAlign: "center", paddingTop: "40vh" }}>Loading...</div>;
  if (error)   return <div style={{ color: "red", textAlign: "center", paddingTop: "40vh" }}>{error}</div>;
  ...
  async function toggleSave(item) {
    ...
    setVideos((prev) =>
      prev.map((v) =>
        v._id === item._id ? { ...v, isSaved: !prevSaved, saveCount: clamp0((v.saveCount ?? 0) + delta) } : v
      )
    );
  ...
  <LogoutButton />
          ...
          <div className="reel" key={id}>
            ...
            <div className="reel-overlay">
              ...
              <div className="reel-content">
                ...
                <button className="reel-btn" onClick={() => navigate(`/food-partner/${item.foodPartner}`)}>
                  visit store
                </button>
              </div>
            </div>
      ...
      <nav className="bottom-nav">
          ...
          {({ isActive }) => (
            <>
              <svg className="icon" viewBox="0 0 24 24" style={{ width: 26, height: 26 }}>
                {isActive ? <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" fill="white" /> : <path d="M3 9.5L12 3l9 6.5v10.5a1 1 0 0 1-1 1h-5v-6h-6v6H4a1 1 0 0 1-1-1V9.5z" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
              </svg>
              home
            </>
          )}
        ...
        <NavLink to="/saved" className={({ isActive }) => `nav-item ${isActive ? "active" : ""}`}>
          {({ isActive }) => (
            <>
              <svg className="icon" viewBox="0 0 24 24" style={{ width: 26, height: 26 }}>
                {isActive ? <path d="M17 3H7c-1.1 0-1.99.9-1.99 2L5 21l7-3 7 3V5c0-1.1-.9-2-2-2z" fill="white" /> : <path d="M5 5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16l-7-3.5L5 21V5z" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
              </svg>
              saved
            </>
          )}
      ...
      {!videos.length && <div style={{ color: "#888", textAlign: "center", padding: 24 }}>No reels yet.</div>}
    </div>
  );
}
